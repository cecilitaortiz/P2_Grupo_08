...
public class RespuestaController implements Initializable {
    ...
    public void imagenRespuesta(String res) {
        try {
            Image imgv = new Image(new FileInputStream(App.pathImg + res.toLowerCase() + ".png"));
            imgRespuesta.setImage(imgv);
        } catch (IOException e) {
            System.out.println(e.getMessage());
        }
    ...
    /**
     * Formatea una lista de nombres de animales, separados por espacios, en una
     * cadena que contiene solo la cantidad especificada de animales, separados
     * por comas.
     *
     * @param cantidad La cantidad de animales a incluir en el resultado.
     * @param animales Una cadena con nombres de animales separados por
     * espacios.
